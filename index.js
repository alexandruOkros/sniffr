...
io.on('connection', function(socket) {
  ...
  socket.on("last_id", function(request) {
    ...
    for(var i = 0; i < connections.length; i++) {
      ...
      if(locations[t_id] === undefined)
        locations[t_id] = { latitude: 51.525921, longitude: -0.139504 }
      if(locations[request.id] === undefined)
        locations[request.id] = { latitude: 51.525921, longitude: -0.139504 }
      var d = distance_f(locations[t_id].latitude, locations[t_id].longitude,
...
app.get('/settings2', function(req, res) {
  // console.log(JSON.stringify(req.query))
  console.log('connection')
